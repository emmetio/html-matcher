...
export default class ContentStreamReader {
	 ...
	 */
	eol() {
		return this.pos >= this.content.length(this.cursor);
	}
	...
	next() {
		if (!this.eof()) {
			...
			if (this.eol()) {
				const next = this.content.next(this.cursor);
				if (next != null) {
					this.cursor = next;
				}
			}

			return this.content.charCodeAt(cursor, pos);
		}


		// const pos = this._next();
		// return pos != null ? this.content.charCodeAt(this.cursor, pos) : undefined;
	}
	...
	peek() {
		if (!this.eof()) {
			if (this.eol()) {
				...
				return this.content.charCodeAt(cursor, 0);
			}

			return this.content.charCodeAt(this.cursor, this.pos);
		}
	}
	...
	back() {
		...
		this.pos = this.content.length(this.cursor) - 1;
		return true;
	}
