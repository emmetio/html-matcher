...
export default class ContentStreamReader {
	...
	get location() {
		return new Point(this.cursor, this.pos)
	}
	 ...
	 */
	eol() {
		return this.pos >= this.content.get(this.cursor).length;
	}
	...
	/**
	 * Moves location by `n` characters back
	 * @param {Number} n Backward offset (default is 1)
	 * @return {Point} Returns new location or `null` if unable to move backward
	 */
	backUp(n) {
		n = n || 1;
