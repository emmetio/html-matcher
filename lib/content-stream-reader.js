...
export default class ContentStreamReader {
	...
	current() {
		...
		if (start.cursor === end.cursor) {
			return this.chunk.substring(start.pos, end.pos);
		}

		let result = this.content.get(start.cursor).slice(start.pos);
		let cursor = start.cursor, chunk;
